...
export default function AdminPlansPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.duration || Number(formData.duration) <= 0)
      return setValidationError("Duration must be greater than 0 minutes"), false
    if (formData.features.length === 0) return setValidationError("At least one feature is required"), false
    setValidationError("")
    return true
  }

  const formatDurationInHours = (minutes: number) => {
    const duration = intervalToDuration({ start: 0, end: minutes * 60 * 1000 })
    return formatDuration(duration, { format: ["hours", "minutes"], zero: true }) // e.g., "2 hours 30 minutes"
  }
  ...
  const handleEdit = (plan: Plan) => {
    ...
    setFormData({
      ...
      price: plan.price.toString(),
      originalPrice: plan.originalPrice?.toString() || "",
      duration: plan.duration?.toString() || "", // Set duration from plan
      features: plan.features,
      isPopular: plan.isPopular,
    })
    setSelectedInheritPlan("none")
            ...
            >
              <CardHeader className="pb-3 flex-shrink-0">
                ...
                <div className="text-sm text-slate-600 mt-2 flex items-center gap-1">
                  ...
                  Duration: {formatDurationInHours(plan.duration || 0)}
                </div>
              </CardHeader>
              <CardContent className="space-y-3 flex-1">
                <Separator className="bg-slate-100" />
                ...
                {planToDelete && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3 space-y-1">
                    ...
                    <div className="text-red-700 text-xs space-y-1">
                      <p>• Plan: {planToDelete.name}</p>
                      <p>• Price: ₹{planToDelete.price.toLocaleString()}</p>
                      <p>• Duration: {formatDurationInHours(planToDelete.duration || 0)}</p>
                      <p>• Features: {planToDelete.features.length}</p>
                      <p>• Status: {planToDelete.isActive ? "Active" : "Inactive"}</p>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              type="number"
              step="0.5"
              value={formData.duration ? (Number(formData.duration) / 60).toString() : ""}
              onChange={(e) => setFormData({ ...formData, duration: (Number(e.target.value) * 60).toString() })}
              placeholder="e.g., 2.5"
              className="border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm pointer-events-auto"
            />
            <p className="text-xs text-slate-500">Enter duration in hours (e.g., 2.5 for 2 hours 30 minutes)</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <Checkbox
