...
//         </CardHeader>
//         <CardContent>
//           {!booking.paymentId || booking.paymentId.status === "pending" ? (
//             <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
//               <div className="flex items-start space-x-3">
//                 <AlertTriangle className="h-5 w-5 text-yellow-600 mt-0.5" />
//                 <div>
...
//                     Payment {booking.paymentId.status.charAt(0).toUpperCase() + booking.paymentId.status.slice(1)}
//                   </p>
//                 </div>
//               </div>
//               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
//                     <Badge
//                       className={`${
//                         booking.paymentId.status === "success"
//                           ? "bg-green-500"
//                           : booking.paymentId.status === "failed" || booking.paymentId.status === "cancelled"
//                           ? "bg-red-500"
//                           : "bg-yellow-500"
//                       } text-white`}
//                     >
//                       {booking.paymentId.status.charAt(0).toUpperCase() + booking.paymentId.status.slice(1)}
//                     </Badge>
//                   </div>
//                 </div>
//                 <div className="space-y-3">
...
import MapModal from "../components/MapModal";
import {
  User, Navigation, Car, AlertTriangle, CreditCard, CheckCircle, ArrowLeft,
  Clock, Calendar, CheckCheck, RefreshCw, Loader2, Receipt, FileText, MapPin,
  DollarSign, Timer, Star,
} from "lucide-react";
import { useParams, useNavigate } from "react-router-dom";
import {
  RoadsideStatusMech, useGenerateQuotationMutation, useRoadsideServiceDetailsQuery,
  useRoadsideStatusUpdateMutation,
} from "../../../services/mechanicServices/roadsideApi";
import BookingDetailsShimmer from "../components/shimmer/BookingDetailsShimmer";
import QuotationModal, { QuotationData } from "../components/service/QuotationModal";
import { initSocket } from "@/lib/socket";
import { useGeolocation } from "@/hooks/useGeolocation";
...
export default function BookingDetails() {
  ...
  useEffect(() => {
    socket.current = initSocket();
    socket.current.on("roadside_assistance_changed", refetch);
    return () => {
      socket.current?.off("roadside_assistance_changed");
    };
  }, [refetch]);
  ...
  if (isLoading) return <BookingDetailsShimmer />;
  if (isError) {
    navigate("/mechanic/404");
    return null;
  }
  ...
  const booking = data?.data;
  const isPaymentPending = !booking.paymentId || booking.paymentId.status === "pending";
  ...
  return (
    <div className="min-h-screen p-4 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <Card className="shadow-sm border-0 bg-white">
          ...
          <CardContent className="pt-0">
            ...
            {(booking.status === "assigned" || booking.status === "on_the_way") && (
              <Button onClick={handleNavigate} className="w-full bg-blue-600 hover:bg-blue-700 mt-4 h-9">
                <Navigation className="h-4 w-4 mr-2" />
                Start Navigate
              </Button>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {booking.endedAt && (
                <div className="flex items-center space-x-3 p-3 bg-green-50 rounded-lg">
                  ...
                  <div>
                    <p className="text-sm text-gray-500">Total Duration</p>
                    <p className="font-medium text-gray-900">{calculateDuration(booking.createdAt, booking.endedAt)}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <Card className="shadow-sm border-0 bg-white mb-6">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="text-center p-4 bg-white rounded-lg shadow-sm">
                <div className="text-2xl font-bold text-blue-600">
                  {booking.startedAt && booking.endedAt ? calculateDuration(booking.startedAt, booking.endedAt) : "-"}
                </div>
                <div className="text-sm text-gray-600">Work Duration</div>
              </div>
              <div className="text-center p-4 bg-white rounded-lg shadow-sm">
      ...
      <div className="space-y-3">
        {booking.status === "assigned" && (
          <>
            <Button onClick={handleNavigate} className="w-full bg-blue-600 hover:bg-blue-700 h-10" disabled={isUpdating}>
              <Navigation className="h-4 w-4 mr-2" />
              Start Navigate
            </Button>
            <Button
        ...
        {booking.status === "analysing" && (
          <>
            <QuotationModal isOpen={isModalOpen} onOpenChange={setIsModalOpen} onSubmit={onQuotationSubmit} />
            <Button onClick={() => setIsModalOpen(true)} className="w-full bg-blue-600 hover:bg-blue-700 h-10">
              <CheckCheck className="h-4 w-4 mr-2" />
              Generate Quotation
            </Button>
          </>
        ...
        {booking.status === "in_progress" && !isPaymentPending && (
          <Button
            onClick={() => handleStatusChange(booking._id, "completed")}
        ...
        {booking.status === "in_progress" && isPaymentPending && (
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
            <div className="flex items-center space-x-3">
              <Clock className="h-5 w-5 text-yellow-600" />
              <p className="text-yellow-800 font-medium">Waiting for Payment</p>
            </div>
            <p className="text-sm text-yellow-700 mt-1">Please wait for the customer to complete the payment before marking the job as completed.</p>
          </div>
        )}
        {booking.status === "completed" && (
          <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
