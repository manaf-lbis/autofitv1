    ...
    subItems: [
      {
        ...
        href: "/mechanic/jobs/roadside-assistance",
      },
      {
        id: "pre-trip-checkup",
        label: "Pre-Trip Checkup",
        href: "/mechanic/jobs/pre-trip-checkup",
      },
      {
        id: "live-assistance",
        label: "Live Assistance",
        href: "/mechanic/jobs/live-assistance",
      },
    ],
  },
  {
...
export default function MechanicDashboard() {
  ...
  const [lastScrollY, setLastScrollY] = useState(0);
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [isJobsExpanded, setIsJobsExpanded] = useState(false); // New state for Jobs sub-menu
  const navigate = useNavigate();
  const location = useLocation();
  const dropdownRef = useRef<HTMLDivElement>(null);
  const { handleLogout } = useLogoutHandler();
  ...
  const getPageTitle = () => {
    ...
    if (activeItem?.id === "jobs") {
      ...
      return activeSubItem?.label || "Jobs";
    }
    ...
    if (activeItem?.id === "jobs") {
      ...
      switch (activeSubItem?.id) {
        case "roadside-assistance":
          return "Handle emergency roadside service requests";
        case "pre-trip-checkup":
          return "Perform vehicle inspections before trips";
        case "live-assistance":
          return "Provide real-time support for customers";
        default:
          return "Manage your service requests and work orders";
      }
    }
              ...
              >
                ...
                {item.subItems && (
                  <ChevronRight
                    className={cn(
                      "h-4 w-4 text-gray-500 transition-transform duration-200",
                      isJobsExpanded && item.id === "jobs" && "rotate-90"
                    )}
                  />
                )}
              </button>
              {item.subItems && isJobsExpanded && item.id === "jobs" && (
