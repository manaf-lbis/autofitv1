  ...
  const [setRead] = useNotificationReadMutation()
    ...
    useEffect(() => { 
      socket.on("notification", (data) => {
        notify('Notification',data.message,new Date().toLocaleTimeString("en-IN", {
        hour: "2-digit",
        minute: "2-digit",
        hour12: true, 
      }))
        ...
        <div className="absolute right-0 mt-2 w-80 sm:w-96 md:w-80 lg:w-96 max-w-[calc(100vw-2rem)] bg-white border border-gray-200 rounded-lg shadow-lg z-50">
          ...
          <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-100">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <span className="text-xs sm:text-sm text-gray-500">{unreadCount} new</span>
                    ...
                    <div className="flex items-start gap-2 sm:gap-3">
                      <div className="flex-shrink-0">
                        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-200 rounded-full flex items-center justify-center">
                          ...
                          {
                            notification.senderName ? <User className="h-3 w-3 sm:h-4 sm:w-4 text-gray-600" /> : <Bell className="h-3 w-3 sm:h-4 sm:w-4 text-gray-600" />
                          }
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 text-xs text-gray-500">
                          ...
                          <span className="text-xs">{formatTimeToNow(notification.createdAt)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
