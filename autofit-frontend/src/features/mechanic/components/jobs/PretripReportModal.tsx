...
interface BookingData {
  ...
  plan: { name: string; price: number; originalPrice: number; schedule: string }
  payment: { paymentId: string; mode: string; amount: number }
}
...
export function PretripReportModal({ isOpen, onClose, bookingData }: ReportModalProps) {
  ...
  const [mechanicNotesError, setMechanicNotesError] = useState<string>("")
  ...
  useEffect(() => {
    if (isOpen) {
      initializeCheckup()
      // Reset errors and messages when modal opens
      setCheckupErrors({})
  ...
  const updateCheckupItem = (index: number, field: keyof CheckupItem, value: any) => {
    ...
    // Clear specific error when item is updated
    if (field === "condition") {
      setCheckupErrors((prev) => {
        const newErrors = { ...prev }
        if (newErrors[index]) {
          delete newErrors[index].condition
        }
        return newErrors
      })
    }
  ...
  const validateCheckup = () => {
    ...
    checkupItems.forEach((item, index) => {
      if (!item.condition) {
        newCheckupErrors[index] = { ...newCheckupErrors[index], condition: "Condition is required" }
        isValid = false
      }
    })
        ...
        <DialogContent className="w-full h-full max-w-none max-h-none m-0 p-0 rounded-none sm:max-w-6xl sm:h-[95vh] sm:max-h-[95vh] sm:rounded-lg sm:m-auto overflow-hidden">
          ...
          <div className="sticky top-0 bg-white border-b border-gray-200 p-4 sm:p-6 z-10">
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="sm" onClick={editReport} className="p-2">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={editReport} className="hidden sm:flex bg-transparent">
                ...
                >
                  {isSaving ? (
                    ...
                    <>
                      <Save className="h-4 w-4 mr-1" /> <span className="hidden sm:inline">Save Report</span>
            ...
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-gray-600 mt-1">
                      Completed on {new Date().toLocaleDateString()} at {bookingData.plan.schedule}
                    </p>
                  </div>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                {checkupItems.map((item, index) => (
                  <div
                    key={index}
                    className="border border-gray-200 rounded-lg p-3 sm:p-4 hover:bg-gray-50 transition-colors bg-white"
                  >
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="w-full h-full max-w-none max-h-none m-0 p-0 rounded-none sm:max-w-6xl sm:h-[95vh] sm:max-h-[95vh] sm:rounded-lg sm:m-auto overflow-hidden">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 p-4 sm:p-6 z-10">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={handleClose} className="p-2 sm:hidden">
              <X className="h-4 w-4" />
            </Button>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs sm:text-sm text-gray-600 truncate">
                {bookingData.vehicle.registration} • {bookingData.vehicle.brand} {bookingData.vehicle.model} •{" "}
              ...
              {isContentLoading ? (
                <>
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div
                      key={i}
                      className="border border-gray-200 rounded-lg p-3 sm:p-4 space-y-3 bg-white animate-pulse"
                    >
                      <div className="h-4 w-3/4 bg-gray-200 rounded"></div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="h-9 bg-gray-200 rounded"></div>
                        <div className="h-9 bg-gray-200 rounded"></div>
                        <div className="col-span-full h-9 bg-gray-200 rounded"></div>
                      </div>
                    </div>
                  ))}
                </>
                ...
                <>
                  {checkupItems.map((item, index) => (
                    <div key={index} className="border border-gray-200 rounded-lg p-3 sm:p-4 space-y-3 bg-white">
                      <h4 className="font-medium text-gray-900 text-sm sm:text-base">{item.name}</h4>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) => updateCheckupItem(index, "condition", value)}
                          >
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4 sm:p-6">
                  <Textarea
                    ...
                    onChange={(e) => {
                      ...
                      setMechanicNotesError("") // Clear error on change
                    }}
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4 sm:p-6">
          <div className="flex gap-3">
            <Button variant="outline" onClick={handleClose} className="flex-1 text-sm bg-transparent">
              Cancel
            </Button>
            <Button onClick={completeCheckup} className="flex-1 bg-green-600 hover:bg-green-700 text-sm">
