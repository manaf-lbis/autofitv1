        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden lg:flex items-center space-x-1">
              {navItems.map((item) => (
                <Link to={item.href} key={item.name}>
                  <div className="relative px-4 py-2 text-gray-700 hover:text-gray-900 transition-colors duration-200 rounded-lg hover:bg-gray-50 font-medium">
                    {item.name}
                  </div>
                </Link>
              ))}
            ...
            <div className="hidden lg:flex items-center space-x-4">
              ...
              {!isAuthenticated || !user ? (
                <Link to="/user/login">
                  <button className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-all duration-200 shadow-sm hover:shadow-md whitespace-nowrap">
                    Get Started
                  </button>
                </Link>
              ) : (
        ...
        <div className="lg:hidden bg-white border-b border-gray-200 shadow-lg">
          <div className="px-4 py-6 space-y-4 max-h-[calc(100vh-4rem)] overflow-y-auto">
            ...
            {isAuthenticated && user ? (
              ...
              <Link to="/user/login" onClick={() => setIsMenuOpen(false)}>
                <button className="w-full px-4 py-3 bg-blue-600 text-white rounded-xl font-medium transition-all duration-200 shadow-sm hover:shadow-md mb-6">
                  Get Started
                </button>
              </Link>
            )}
            ...
            <div className="space-y-2">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="flex items-center space-x-3 px-4 py-3 text-gray-700 hover:text-gray-900 hover:bg-gray-50 rounded-xl transition-all duration-200 font-medium"
                  onClick={() => setIsMenuOpen(false)}
                >
            ...
            {isAuthenticated && user && (
              <div className="pt-6 border-t border-gray-200 space-y-2">
                <Link to="/user/profile" onClick={() => setIsMenuOpen(false)}>
                  <button className="w-full text-left px-4 py-3 text-gray-700 hover:text-gray-900 hover:bg-gray-50 rounded-xl transition-all duration-200 font-medium">
                    Profile Settings
                  </button>
                </Link>
                {isLoading ? (
                  <div className="w-full flex gap-2 items-center px-4 py-3 text-red-600">
                    <Loader2 className="animate-spin" size={16} />
                    <span>Logging out...</span>
                  </div>
                ) : (
                  <button
                    className="w-full text-left px-4 py-3 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-xl transition-all duration-200 font-medium"
                    onClick={async () => {
                      await logout().unwrap()
                      dispatch(clearUser())
                      window.location.href = "/auth/user/login"
